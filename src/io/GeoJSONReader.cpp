    ...
    try {
        ...
        if (type == "Feature") {
            ...
            return GeoJSONFeatureCollection{
                std::vector<GeoJSONFeature>{GeoJSONFeature{std::move(g), std::map<std::string, GeoJSONValue>{}}}};
        }
    } catch (json::exception& ex) {
        throw ParseException("Error parsing JSON", ex.what());
    }
...
std::unique_ptr<geom::Geometry> GeoJSONReader::readFeatureForGeometry(const nlohmann::json& j) const {
    const auto& geometryJson = j.at("geometry");
    auto geometry = readGeometry(geometryJson);
    return geometry;
}

GeoJSONFeature GeoJSONReader::readFeature(const nlohmann::json& j) const {
    const auto& geometryJson = j.at("geometry");
    const auto& properties = j.at("properties");
    return GeoJSONFeature{readGeometry(geometryJson), readProperties(properties)};
}

std::map<std::string, GeoJSONValue> GeoJSONReader::readProperties(const nlohmann::json& p) const {
...
GeoJSONValue GeoJSONReader::readProperty(const nlohmann::json& value) const {
    if (value.is_string()) {
...
std::unique_ptr<geom::Geometry> GeoJSONReader::readFeatureCollectionForGeometry(const nlohmann::json& j) const {
...
GeoJSONFeatureCollection GeoJSONReader::readFeatureCollection(const nlohmann::json& j) const {
    const auto& featuresJson = j.at("features");
...
std::unique_ptr<geom::Geometry> GeoJSONReader::readGeometry(const nlohmann::json& j) const {
    const std::string& type = j.at("type");
    if (type == "Point") {
        return readPoint(j);
...
geom::Coordinate GeoJSONReader::readCoordinate(const std::vector<double>& coords) const {
    if (coords.size() == 1) {
...
std::unique_ptr<geom::Point> GeoJSONReader::readPoint(const nlohmann::json& j) const {
...
std::unique_ptr<geom::LineString> GeoJSONReader::readLineString(const nlohmann::json& j) const {
...
std::unique_ptr<geom::Polygon> GeoJSONReader::readPolygon(const nlohmann::json& j) const {
...
std::unique_ptr<geom::MultiPoint> GeoJSONReader::readMultiPoint(const nlohmann::json& j) const {
    const auto& coords = j.at("coordinates").get<std::vector<std::vector<double>>>();
...
std::unique_ptr<geom::MultiLineString> GeoJSONReader::readMultiLineString(const nlohmann::json& j) const {
...
std::unique_ptr<geom::MultiPolygon> GeoJSONReader::readMultiPolygon(const nlohmann::json& j) const {
    const auto& multiPolygonCoords =
        j.at("coordinates").get<std::vector<std::vector<std::vector<std::vector<double>>>>>();
...
std::unique_ptr<geom::GeometryCollection> GeoJSONReader::readGeometryCollection(const nlohmann::json& j) const {
